...
@TRANSACTION_FILE_NAME,@RUN_DATE_TIME,@RUN_RESULT,@DATA_MOVEMENT_INFORMATION,@PAYMENT_DAY,@TYPE_OF_ALLOCATION,@ALLOCATED_QUANTITY,@ALLOCATED_MONEY,@ALLOCATED_COMPLETION_DATE)";
        string strGetUncompleteReceiptDetail = @"SELECT RCP.SEQ_NO,RCP.CUSTOMER_NAME,RCP.DEPOSIT_AMOUNT,RCP.ALLOCATED_MONEY,RCP.ALLOCATED_QUANTITY,Customer.COMPANY_NO_BOX 
                                                FROM 
                                                (SELECT * FROM RECEIPT_DETAILS
                                                WHERE ISNULL([ALLOCATED_COMPLETION_DATE],'') = ''
                                                AND RUN_RESULT = 1 AND [DEPOSIT_AMOUNT] > ISNULL([ALLOCATED_MONEY],0))RCP
                                                LEFT JOIN CUSTOMER_MASTER Customer
                                                ON 
                                                (
                                                REPLACE(CONVERT(nvarchar(50),Customer.BILL_BANK_ACCOUNT_NAME_1),' ','') = REPLACE(CONVERT(nvarchar(50),RCP.CUSTOMER_NAME),' ','')
                                                OR
                                                REPLACE(CONVERT(nvarchar(50),Customer.BILL_BANK_ACCOUNT_NAME_2),' ','') = REPLACE(CONVERT(nvarchar(50),RCP.CUSTOMER_NAME),' ','')
                                                OR
                                                REPLACE(CONVERT(nvarchar(50),Customer.BILL_BANK_ACCOUNT_NAME_3),' ','') = REPLACE(CONVERT(nvarchar(50),RCP.CUSTOMER_NAME),' ','')
                                                OR
                                                REPLACE(CONVERT(nvarchar(50),Customer.BILL_BANK_ACCOUNT_NAME_4),' ','') = REPLACE(CONVERT(nvarchar(50),RCP.CUSTOMER_NAME),' ','')
                                                )
                                                WHERE ISNULL(Customer.COMPANY_NO_BOX,'') <> ''
